...
import warnings
from yolov5s_bdd100k.deep_sort import build_tracker
# from utils.draw import draw_boxes
from utils.draw import pil_draw_box,pil_draw_box_2
...
class VideoTracker(object):
    def __init__(self, cfg):
        ...
        self.trafficLine=[]
        self.idx_frame=0
        self.carSpeedLimit=5
        self.filename=0
        self.trafficJamLimit=10
        self.rett=False
        self.carLicense={}
    ...
    def run(self,sinImg,sin2,sinInfo,sinIllegal,config):
        self.carSpeedLimit =     40
        self.trafficJamLimit =   10
        self.yolov5weight =      './runs/best.pt'
        self.detector =          YOLO(self.yolov5weight)
        self.video_path =        'inference/input/01.mp4'
        filepath, filename =      os.path.split(self.video_path)
        self.outputDir =         'inference/output/'+filename.split('.')[0]+"/"
        ...
        while self.vdo.grab():
            ...
            print('\tyolo所用时间{}s'.format(round(time.time()-yolotime,4)))
            # cv2.imshow('img',ori_im)
            # cv2.waitKey(0)
            deeptime = time.time()
            outputs = self.deepsort.update(bbox_xywh, cls_conf, im)
            print('\tdeepsort所用时间{}s'.format(round(time.time() - deeptime, 4)))
            ...
            if len(self.trafficLine1)>0 or len(self.trafficLine2)>0:
                if len(self.trafficLine1)>0:
                    plot_one_box(self.trafficLine1, ori_im, label="TrafficLine1", color=(105,210,204), line_thickness=3)
                if len(self.trafficLine2)>0:
                    plot_one_box(self.trafficLine2, ori_im, label="TrafficLine2", color=(105,210,204), line_thickness=3)
                ...
                plot_one_box(self.trafficLine, ori_im, label=label, color=(105,210,204), line_thickness=3)
            ...
            print('正在处理：第{}帧\t总帧数：{}\t所用时间：{}s'.format(self.idx_frame,int(self.frameAll),round(end-start,4)))
        timall = time.time() - timall
        print('总用时{}s'.format(round(timall,4)))
...
def parse_args():
    ...
    parser.add_argument("--VIDEO_PATH", default='./inference/input/01.mp4',type=str)
...
if __name__ == "__main__":
    ...
    vdo_trk.run(sin, sin2, sinInfo, sinIllegal, config)